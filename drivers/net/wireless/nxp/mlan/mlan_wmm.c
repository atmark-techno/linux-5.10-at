 ...
 */
static t_u8
is_station_wmm_enabled(mlan_private *priv, t_u8 *mac)
{
 ...
 */
static int
wlan_is_wmm_enabled(mlan_private *priv, t_u8 *ra)
{
	int ret = MFALSE;
	ENTER();
#ifdef UAP_SUPPORT
...
wlan_ret_wmm_get_status(pmlan_private priv, t_u8 *ptlv, int resp_len)
{
	...
	while (resp_len >= (int)sizeof(ptlv_hdr->header)) {
		...
		case TLV_TYPE_WMMQSTATUS:
			...
			/* Pick the minimum among these to avoid array out of bounds */
			ptlv_wmm_q_status->queue_index =
				MIN(ptlv_wmm_q_status->queue_index,
				    MAX_AC_QUEUES);
